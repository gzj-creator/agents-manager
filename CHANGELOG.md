...
- 保持标签、客户端和同步模式选择框原有宽度，仅压缩控件高度，减少垂直空间占用。
...
- 固定桌面主页面视口高度，避免 Skills 和 Memory 页面继续向下滚动；列表与详情改为各自内部滚动，窄屏保留自然页面滚动。

### Tests

- 补充主壳层、Skills 和 Memory 滚动约束回归测试，并通过桌面端测试与 Vite 生产构建。
